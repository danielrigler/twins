...
function Mirror.init(osc_positions_ref, lfo_ref, voice_params)
    ...
    Mirror.lfo = lfo_ref
    Mirror.voice_params = voice_params
    params_lookup = params.lookup
    lfo_targets = lfo_ref and lfo_ref.lfo_targets
    _LFO_KEYS    = lfo_ref.keys.lfo
    _TARGET_KEYS = lfo_ref.keys.target
    _SHAPE_KEYS  = lfo_ref.keys.shape
    _FREQ_KEYS   = lfo_ref.keys.freq
    _DEPTH_KEYS  = lfo_ref.keys.depth
    _OFFSET_KEYS = lfo_ref.keys.offset
    target_lookup = {}
