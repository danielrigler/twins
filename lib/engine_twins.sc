...
readBuf { arg i, path; if(buffersL[i].notNil && buffersR[i].notNil,{ if(File.exists(path),{ var numChannels=SoundFile.use(path.asString(),{|f| f.numChannels}); voicesUsingLiveBuffer[i] = false; Buffer.readChannel(context.server,path,0,-1,[0],{|b| voices[i].set(\buf_l,b); buffersL[i].free; buffersL[i]=b; if(numChannels>1,{ Buffer.readChannel(context.server,path,0,-1,[1],{|b| voices[i].set(\buf_r,b); buffersR[i].free; buffersR[i]=b; voices[i].set(\t_reset_pos,1); nornsAddr.sendMsg("/twins/buffer_loaded",i+1); }); },{ voices[i].set(\buf_r,b); buffersR[i].free; buffersR[i]=b; voices[i].set(\t_reset_pos,1); nornsAddr.sendMsg("/twins/buffer_loaded",i+1); }); }); },{ nornsAddr.sendMsg("/twins/buffer_loaded",i+1); }); }); }

unloadAll { fork { 2.do({ arg i; var newBufL = Buffer.alloc(context.server, context.server.sampleRate.asInteger); var newBufR = Buffer.alloc(context.server, context.server.sampleRate.asInteger); context.server.sync; if(buffersL[i].notNil, { buffersL[i].free; }); if(buffersR[i].notNil, { buffersR[i].free; }); buffersL.put(i, newBufL); buffersR.put(i, newBufR); if(voices[i].notNil, { voices[i].set(\buf_l, newBufL, \buf_r, newBufR, \t_reset_pos, 1); }); voicesUsingLiveBuffer[i] = false; liveInputBuffersL[i].zero; liveInputBuffersR[i].zero; if(liveInputRecorders[i].notNil, { liveInputRecorders[i].free; liveInputRecorders[i] = nil; }); }); wobbleBuffer.zero; glitchBuffer.zero; }; }
