...
var <buffersL, <buffersR, wobbleBuffer, glitchBuffer, mixBus, postFxBus, shimmerBus, parallelBus, <voices, bufSine, pg, <liveInputBuffersL, <liveInputBuffersR, <liveInputRecorders, <liveRecPosBuses, o, o_output, o_rec, o_grain, o_voice_peak, mixToParallelRouter, parallelToPostFxRouter, finalOutputRouter;
var <outputRecordBuffer, <outputRecorder, outputBufferLength = 8, currentOutputWritePos, liveBufferAllocGeneration = 0, grainEnvs, pitchScaleBuffers, pitchScaleLengths, nornsAddr;
...
readBuf { arg i, path; if(buffersL[i].notNil && buffersR[i].notNil,{ if(File.exists(path),{ var numChannels=SoundFile.use(path.asString(),{|f| f.numChannels}); Buffer.readChannel(context.server,path,0,-1,[0],{|b| voices[i].set(\buf_l,b); buffersL[i].free; buffersL[i]=b; if(numChannels>1,{ Buffer.readChannel(context.server,path,0,-1,[1],{|b| voices[i].set(\buf_r,b); buffersR[i].free; buffersR[i]=b; voices[i].set(\t_reset_pos,1); nornsAddr.sendMsg("/twins/buffer_loaded",i+1); }); },{ voices[i].set(\buf_r,b); buffersR[i].free; buffersR[i]=b; voices[i].set(\t_reset_pos,1); nornsAddr.sendMsg("/twins/buffer_loaded",i+1); }); }); },{ nornsAddr.sendMsg("/twins/buffer_loaded",i+1); }); }); }

unloadAll { fork { 2.do({ arg i; var newBufL = Buffer.alloc(context.server, context.server.sampleRate.asInteger); var newBufR = Buffer.alloc(context.server, context.server.sampleRate.asInteger); context.server.sync; if(buffersL[i].notNil, { buffersL[i].free; }); if(buffersR[i].notNil, { buffersR[i].free; }); buffersL.put(i, newBufL); buffersR.put(i, newBufR); if(voices[i].notNil, { voices[i].set(\buf_l, newBufL, \buf_r, newBufR, \t_reset_pos, 1); }); liveInputBuffersL[i].zero; liveInputBuffersR[i].zero; if(liveInputRecorders[i].notNil, { liveInputRecorders[i].free; liveInputRecorders[i] = nil; }); }); wobbleBuffer.zero; glitchBuffer.zero; }; }
...
alloc {
        ...
        SynthDef(\liveDirect, {
            arg out, pan, gain, cutoff, hpf, low_gain, mid_gain, high_gain, isMono, lpfgain, voice;
        ...
        SynthDef(\glitch, {
            ...
            var sig, bufFrames, writePos, trigOn, trigOff, isGlitching;
            var capturePos, chunkLength, chunkStart;
            var pitchShift, isReverse;
            var relPos, bufReadPos, wet_raw;
            var fadeSamples, startRamp, endRamp, loopEnv;
            var wet, glitched;
            sig = In.ar(bus, 2);
            ...
            fadeSamples = (SampleRate.ir * 0.03); 
            relPos = Phasor.ar(trigOn, pitchShift, 0, chunkLength, 0);
            bufReadPos = chunkStart + Select.ar(K2A.ar(isReverse), [relPos, chunkLength - relPos]);
            bufReadPos = bufReadPos.wrap(0, bufFrames - 1);
            ...
            loopEnv = 0.5 - (0.5 * cos(startRamp.min(endRamp) * pi));
            wet = wet_raw * loopEnv;
            glitched = XFade2.ar(sig, wet, isGlitching * 2 - 1);
            ReplaceOut.ar(bus, XFade2.ar(sig, glitched, mix * 2 - 1));
        }).add;
        ...
        this.addCommand("set_live_input", "ii", { arg msg; var voice = msg[1] - 1; var enable = msg[2]; if (enable == 1, { if (liveInputRecorders[voice].notNil, { liveInputRecorders[voice].free; }); liveRecPosBuses[voice].set(-1.0); liveInputRecorders[voice] = Synth.new(\liveInputRecorder, [ \bufL, liveInputBuffersL[voice], \bufR, liveInputBuffersR[voice], \mix, liveBufferMix, \voice, voice, \recPosBus, liveRecPosBuses[voice].index ], context.xg, 'addToHead'); voices[voice].set( \buf_l, liveInputBuffersL[voice], \buf_r, liveInputBuffersR[voice], \rec_pos_bus, liveRecPosBuses[voice].index, \t_reset_pos, 1); }, { if (liveInputRecorders[voice].notNil, { liveInputRecorders[voice].free; }); liveInputRecorders[voice] = nil; liveRecPosBuses[voice].set(-1.0); voices[voice].set(\rec_pos_bus, -1); }); });
        ...
        this.addCommand("save_output_buffer", "s", { arg msg; this.writeOutputBuffer(msg[1], true); });
        this.addCommand("save_output_buffer_only", "s", { arg msg; this.writeOutputBuffer(msg[1], false); });
        ...
        o_grain = OSCFunc({ |msg| var voice, pos, size; voice = msg[3].asInteger; pos = msg[4]; size = msg[5]; nornsAddr.sendMsg("/twins/grain_pos", voice, pos, size);}, '/grain_pos', context.server.addr);
