...
local function store_scene(track, scene)
    ...
    local global_params = {"delay_mix", "delay_time", "delay_feedback", "reverb_mix", "shimmer_mix", "tape_mix"}
...
local function is_lfo_active_for_param(param_name)
    for i = 1, 16 do
        local target_index = params:get(i.. "lfo_target")
        if lfo.lfo_targets[target_index] == param_name and params:get(i.. "lfo") == 2 then return true, i end
    end
    return false, nil
end
...
local function apply_morph()
    ...
    for param in pairs(all_params) do
        ...
        if valueA and valueB then params:set(param, valueA + (valueB - valueA) * t)
        elseif valueA then params:set(param, valueA)
        elseif valueB then params:set(param, valueB)
        end
    end
    -- Morph global parameters
    local global_params = {"delay_mix", "delay_time", "delay_feedback", "reverb_mix", "shimmer_mix", "tape_mix"}
    for i = 1, #global_params do
        local param = global_params[i]
...
local function setup_params()
    ...
    params:add_option("scene_mode", "Scene Mode", {"off", "on"}, 1) params:set_action("scene_mode", function(value) current_scene_mode = (value == 2) and "on" or "off" if current_scene_mode == "on" then initialize_scenes_with_current_params() params:set("morph_amount", 0) end end) params:hide("scene_mode")
